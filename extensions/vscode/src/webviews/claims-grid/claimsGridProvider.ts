...
import { Claim, ClaimsDocument } from '../../types';
...
export class ClaimsGridProvider implements vscode.WebviewViewProvider {
    ...
    public highlightClaim(claimId: string) {
        ...
        if (claim && this._currentDocument && vscode.window.activeTextEditor) {
            const startPos = this._currentDocument.positionAt(claim.start);
            const endPos = this._currentDocument.positionAt(claim.end);
    ...
    private _classifyClaimType(text: string): Claim['type'] {
        ...
        if (lowerText.includes('controversial') || lowerText.includes('alleged') || lowerText.includes('disputed')) {
            return 'contentious';
        }
    ...
    private _assessRisk(text: string): Claim['risk'] {
        ...
        if (lowerText.includes('alleged') || lowerText.includes('controversial') || lowerText.includes('disputed')) {
            return 'high';
        }
    ...
    private _getHtmlForWebview(webview: vscode.Webview): string {
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'claims-grid.js'));
        const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'claims-grid.css'));
