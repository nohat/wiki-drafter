...
export class PreviewProvider implements vscode.WebviewViewProvider {
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage((data) => {
      switch (data.type) {
        ...
        case "ready":
          console.log(
            "PreviewProvider: Webview ready, current document:",
  ...
  public updatePreview(document: vscode.TextDocument) {
    ...
    this._debounceTimer = setTimeout(() => {
      // Fire and forget; we intentionally don't await inside setTimeout
      void this._renderPreview(document);
    }, 200);
  }
  ...
  private async _renderPreview(document: vscode.TextDocument) {
    ...
    try {
      ...
      const companionEndpoint = config.get<string>(
        "companionService.endpoint",
        "http://localhost:8000",
      );

      const wikitext = document.getText();
      console.log(
        "PreviewProvider: Attempting to render via companion service:",
        companionEndpoint,
      );
      ...
      console.log(
        "PreviewProvider: Using fallback renderer, generated HTML length:",
        basicHtml.length,
      );
  ...
  private _getBasicHtmlPreview(wikitext: string): string {
    ...
    let html = wikitext
      ...
      // Headers: == Title ==
      .replace(/^(={2,6})\s*(.+?)\s*\1$/gm, (match: string, level: string, title: string) => {
      ...
      .replace(
        /<ref[^>]*name\s*=\s*["']([^"']+)["'][^>]*>(.*?)<\/ref>/g,
        '<sup class="reference"><a href="#ref-$1">[$1]</a></sup>',
      )
